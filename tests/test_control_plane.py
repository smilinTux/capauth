...
def test_expiry_and_ttl_fail_closed(expires_at, expected) -> None:
    boundary = RequestBoundary(client_kind=ClientKind.NATIVE, idempotency_key=IDEMPOTENCY_KEY)
...
def test_client_kind_cannot_impersonate_a_human_principal() -> None:
    boundary = RequestBoundary(client_kind=ClientKind.AGENT, idempotency_key=IDEMPOTENCY_KEY)
