"""Tests for the CAPAUTH_LEGACY_UNSIGNED_GRACE_UNTIL exception (card b2399d30).
...
def test_future_flag_never_grants_a_tampered_signature(tmp_path, monkeypatch, stub_token_signing):
    ...
    monkeypatch.setenv(LEGACY_UNSIGNED_GRACE_ENV, _future_deadline(days=7))
    _enroll(tmp_path)
    token = issue_token(home=tmp_path, subject=SUBJECT, capabilities=[CAPABILITY], sign=True)
